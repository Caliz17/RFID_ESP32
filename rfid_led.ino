...
// Pin para el LED
#define LED_PIN 2  // Puedes cambiar este valor según la conexión física
...
void loop() {
  ...
  // UID de la tarjeta deseada
  uint8_t tarjetaDeseada[] = { 0xE3, 0x88, 0x7A, 0x12 };
  ...
  if (LeeTarjeta) {
    ...
    // Verifica si el UID coincide con el de la tarjeta deseada
    if (memcmp(uid, tarjetaDeseada, LongitudUID) == 0) {
      digitalWrite(LED_PIN, HIGH); // Enciende el LED
      delay(1000); // Espera un segundo
      digitalWrite(LED_PIN, LOW); // Apaga el LED después de un segundo
    }
